...
<body>
	<input id="thisUserId" value="${sessionScope.userId}" style="display:none"/>
					...
					<div class="panel panel-default"  id="userInfo">
						<div class="panel-body" style="padding:12px">
							<div class="user">
								...
								<div  onclick="uploadImg" class="userimg">
									<a id="my-img" href="#"><img src="${pageContext.request.contextPath}/${ user.userImg}"></a>
								</div>
								</c:if>
								<c:if test="${user.id!=sessionScope.userId}">
								<div  class="userimg">
							...
							<div class="basicdata">
								...
								<div class="userdata">
									<h4>
										<span>实名:</span><span>${user.truename}</span>|
										<span>出生年月:</span><span>${user.birthday}</span>|
										<!-- <span>性别:</span><span>${user.gender==0?'女':'男'}</span> -->
									</h4>
									<h4>
										<span>住址:</span><span>${user.province}-${user.city}</span>| 
						...
						<div class="tab-pane fade in active" id="myData">
							<ul class="list-group new-group">
								...
								<li class="list-group-item">
									<ul class="list-group">
										<li class="list-group-item">
											<c:forEach items="${educationList}" var="e">
											<div class="panel-body education">
												<div class="info">
													<span>时间:</span><span><fmt:formatDate value="${e.startDate }" pattern="yyyy-MM-dd"/>到<fmt:formatDate value="${e.endDate }" pattern="yyyy-MM-dd"/></span>&nbsp;&nbsp;|&nbsp;
													<span>学历:</span><span>${e.education}</span>&nbsp;&nbsp;|&nbsp; 
													<span>学校:</span><span>${e.school}</span>&nbsp;&nbsp;|&nbsp;
													<span>专业:</span><span>${e.profession}</span>
												</div>
	...
	<%@ include file="/WEB-INF/jsp/common/tail.jsp"%>
	</body>
</html>